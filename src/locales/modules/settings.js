...
export default {
  ...
  desc: _('管理 AI Agent面板 的网络、代理和下载源配置', 'Manage AI Agent面板 network, proxy and download source settings', '管理 AI Agent面板 的網路、代理和下載源設定', 'AI Agent面板 のネットワーク、プロキシ、ダウンロードソース設定を管理', 'AI Agent面板 네트워크, 프록시 및 다운로드 소스 설정 관리', 'Quản lý cài đặt mạng, proxy và nguồn tải xuống', 'Gestionar configuración de red, proxy y fuentes de descarga', 'Gerenciar configurações de rede, proxy e fontes de download', 'Управление сетью, прокси и источниками загрузки', 'Gérer les paramètres réseau, proxy et sources de téléchargement', 'Netzwerk-, Proxy- und Download-Einstellungen verwalten'),
  ...
  openclawDir: _('AI Agent 配置目录', 'AI Agent Config Directory', 'AI Agent 設定目錄', 'AI Agent 設定ディレクトリ', 'AI Agent 설정 디렉터리'),
  openclawSearchPaths: _('额外 AI Agent 搜索路径', 'Extra AI Agent Search Paths', '額外 AI Agent 搜尋路徑', '追加の AI Agent 検索パス', '추가 AI Agent 검색 경로'),
  dockerDefaults: _('Docker 默认配置', 'Docker Defaults', 'Docker 預設配置', 'Docker デフォルト設定', 'Docker 기본 설정'),
  openclawCli: _('AI Agent CLI 绑定', 'AI Agent CLI Binding', 'AI Agent CLI 綁定', 'AI Agent CLI バインド', 'AI Agent CLI 바인딩'),
  autostart: _('开机自启', 'Auto Start', '開機自啟', '自動起動', '자동 시작'),
  autostartToggle: _('系统启动时自动运行 AI Agent面板', 'Run AI Agent面板 automatically on system startup', '系統啟動時自動執行 AI Agent面板', 'システム起動時に AI Agent面板 を自動実行', '시스템 시작 시 AI Agent面板 자동 실행'),
  autostartHint: _('开启后，电脑重启时 AI Agent面板 会自动启动并检测 Gateway 状态', 'When enabled, AI Agent面板 will start automatically after reboot and check Gateway status', '開啟後，電腦重啟時 AI Agent面板 會自動啟動並檢查 Gateway 狀態', '有効にすると、PC 再起動後に AI Agent面板 が自動起動し Gateway 状態を確認します', '활성화하면 PC 재시작 후 AI Agent面板이 자동으로 시작되어 Gateway 상태를 확인합니다'),
  ...
  cliBindHint: _('选择面板实际使用的 AI Agent CLI，适用于多版本共存场景', 'Select which AI Agent CLI the panel should use, useful when multiple versions coexist', '選擇面板實際使用的 AI Agent CLI，適用於多版本共存場景', 'パネルが使用する AI Agent CLI を選択、複数バージョン共存時に便利', '패널이 사용할 AI Agent CLI 선택, 여러 버전 공존 시 유용'),
  ...
  proxyHint: _('设置后，npm 安装/升级、版本检测、GitHub/Gitee 更新检查、海外技能源 Skills 等下载类操作会走此代理。自动绕过 localhost 和内网地址。保存后新请求立即生效；如 Gateway 正在运行，建议重启一次服务。', 'Once set, npm install/upgrade, version checks, GitHub/Gitee update checks, overseas skill catalog downloads, and similar operations will use this proxy. Localhost and LAN addresses are auto-bypassed. Takes effect immediately; if Gateway is running, consider restarting the service.', '設定后，npm 安裝/升級、版本檢測、GitHub/Gitee 更新檢查、海外技能源 Skills 等下載類操作會走此代理。自動繞過 localhost 和內網位址。儲存后新請求立即生效；如 Gateway 正在執行，建議重啟一次服務。', '設定後、npm インストール/アップグレード、バージョンチェック、GitHub/Gitee 更新チェック、海外スキルカタログのダウンロードなどはこのプロキシを使用します。localhost と LAN アドレスは自動バイパスされます。すぐに有効になります。Gateway が実行中の場合はサービスの再起動を検討してください。'),
  ...
  dockerDefaultImage: _('默认 AI Agent 镜像', 'Default AI Agent Image', '預設 AI Agent 映像', 'デフォルト AI Agent イメージ', '기본 AI Agent 이미지'),
  dockerDefaultImagePlaceholder: _('例如 ghcr.io/openclaw/openclaw 或 1186258278/openclaw', 'e.g. ghcr.io/openclaw/openclaw or 1186258278/openclaw', '例如 ghcr.io/openclaw/openclaw 或 1186258278/openclaw', '例: ghcr.io/openclaw/openclaw または 1186258278/openclaw'),
  ...
  dirHint: _('自定义 AI Agent 配置目录路径。修改后需要重启面板生效。目标目录必须存在且包含 openclaw.json。', 'Custom AI Agent config directory path. Restart required after changes. Target directory must exist and contain openclaw.json.', '自定義 AI Agent 設定目錄路徑。修改后需要重啟面板生效。目標目錄必須存在且包含 openclaw.json。', 'カスタム AI Agent 設定ディレクトリパス。変更後は再起動が必要です。対象ディレクトリは存在し、openclaw.json を含む必要があります。'),
