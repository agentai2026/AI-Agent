...
export default {
  ...
  desc: _('安装和配置 AI Agent', 'Install and configure AI Agent', '安裝和設定 AI Agent', 'AI Agent のインストールと設定', 'AI Agent 설치 및 설정', 'Cài đặt và cấu hình AI Agent', 'Instalar y configurar AI Agent', 'Instalar e configurar AI Agent', 'Установка и настройка AI Agent', 'Installer et configurer AI Agent', 'AI Agent installieren und konfigurieren'),
  headerTitle: _('欢迎使用 AI Agent面板', 'Welcome to AI Agent面板', '', 'AI Agent面板 へようこそ', 'AI Agent面板에 오신 것을 환영합니다', 'Chào mừng đến AI Agent面板', 'Bienvenido a AI Agent面板', 'Bem-vindo ao AI Agent面板', 'Добро пожаловать в AI Agent面板', 'Bienvenue sur AI Agent面板', 'Willkommen bei AI Agent面板'),
  headerDesc: _('OpenClaw & Hermes Agent 多引擎 AI 管理面板终端', 'Multi-engine AI management panel terminal (OpenClaw & Hermes Agent)', '', 'OpenClaw & Hermes Agent 多エンジン AI 管理パネル端末', 'OpenClaw & Hermes Agent 멀티 엔진 AI 관리 패널 터미널'),
  ...
  stepNodeHint: _('AI Agent 基于 Node.js 运行，请先安装。', 'AI Agent requires Node.js, please install it first.', 'AI Agent 基於 Node.js 執行，請先安裝。', 'AI Agent は Node.js で動作します。先にインストールしてください。'),
  ...
  macNodeHint: _('macOS 上从 Finder 启动可能找不到 Node.js。试试关掉 AI Agent面板 后从终端启动：', 'On macOS, launching from Finder may not find Node.js. Try closing AI Agent面板 and launching from terminal:', 'macOS 上從 Finder 啟動可能找不到 Node.js。試試關掉 AI Agent面板 后從終端啟動：'),
  ...
  cliAheadWarning: _('检测到当前本地 AI Agent {current} 高于当前面板推荐稳定版 {recommended}，可能存在兼容或稳定性风险。建议稍后到「关于」页回退到推荐版。', 'Detected local AI Agent {current} is ahead of the panel\'s recommended stable version {recommended}. There may be compatibility or stability risks. Consider rolling back to the recommended version on the About page.', '檢測到目前本地 AI Agent {current} 高於目前面板推薦穩定版 {recommended}，可能存在相容或穩定性風險。建議稍后到「關於」頁回退到推薦版。'),
  ...
  customDirTitle: _('自定义 AI Agent 配置目录', 'Custom AI Agent Config Directory', '自定義 AI Agent 設定目錄', 'カスタム AI Agent 設定ディレクトリ', '사용자 정의 AI Agent 설정 디렉터리'),
  ...
  customDirNotice: _('这里只影响配置文件与状态目录，不会替代上方选择的 AI Agent CLI 可执行文件。', 'This only changes the config/state directory and does not replace the AI Agent CLI executable selected above.', '這裡只會影響設定檔與狀態目錄，不會取代上方選擇的 AI Agent CLI 可執行檔。', 'ここで変更されるのは設定/状態ディレクトリのみで、上で選択した AI Agent CLI 実行ファイルは置き換えられません。'),
  ...
  aiAssistant: _('AI 助手', 'AI Assistant', '', 'AI アシスタント', 'AI 어시스턴트', 'Trợ lý AI', 'Asistente IA', 'Assistente IA', 'ИИ-ассистент', 'Assistant IA', 'KI-Assistent'),
  ...
  installHint: _('点击安装后，将默认安装当前 OpenClaw 面板 版本绑定的推荐稳定版；如需升降级，可稍后到「关于」页面切换版本。', 'Installation will use the recommended stable version bound to this OpenClaw panel release. To change versions later, go to the About page.', '点擊安裝后，將預設安裝目前 OpenClaw 面板 版本綁定的推薦穩定版；如需升降級，可稍后到「關於」頁面切換版本。'),
  ...
  methodHintR2: _('从项目 CDN 下载独立安装包，自带 Node.js 运行时，无需 npm。国内下载速度最快。', 'Downloads standalone package from CDN with bundled Node.js runtime, no npm needed. Fastest for China.', '從專案 CDN 下載獨立安裝包，自帶 Node.js 執行時，無需 npm。國內下載速度最快。'),
  ...
  envHintTitle: _('找不到已安装的 AI Agent？', 'Can\'t find installed AI Agent?', '找不到已安裝的 AI Agent？'),
  envHintDesc: _('AI Agent面板 桌面版只能直接管理<strong>当前 Windows / macOS / Linux 本机</strong>里的 AI Agent。若你的 AI Agent 实际装在 WSL、Docker 或远程服务器里，请把下面内容视为<strong>高级替代方案</strong>，而不是普通本机安装的默认路径。', 'AI Agent面板 desktop can only directly manage AI Agent installed on the <strong>current local Windows / macOS / Linux machine</strong>. If your AI Agent actually lives in WSL, Docker or a remote server, treat the options below as <strong>advanced alternatives</strong>, not the default local setup path.', 'AI Agent面板 桌面版只能直接管理<strong>目前 Windows / macOS / Linux 本機</strong>裡的 AI Agent。若你的 AI Agent 實際裝在 WSL、Docker 或遠端伺服器裡，請把下面內容視為<strong>進階替代方案</strong>，而不是普通本機安裝的預設路徑。'),
  envHintWsl: _('WSL (Windows 子系统)', 'WSL (Windows Subsystem for Linux)', 'WSL (Windows 子系統)'),
  envHintWslDesc: _('AI Agent 装在 WSL 里，Windows 侧无法访问', 'AI Agent in WSL is not accessible from Windows side', 'AI Agent 裝在 WSL 里，Windows 側無法訪問'),
  ...
  envHintInstallManage: _('高级替代方案：在对应环境里使用 AI Agent面板 Web 版', 'Advanced alternative: use AI Agent面板 Web inside that environment', '進階替代方案：在對應環境裡使用 AI Agent面板 Web 版'),
  envHintLocalReinstall: _('如果你要管理的是当前这台电脑里的 AI Agent，更建议直接使用本页的「安装 OpenClaw」卡片，或在上方手动绑定 CLI 路径。', 'If you want to manage AI Agent on this computer, use the \"Install OpenClaw\" card on this page or bind the CLI path above.', '如果你要管理的是目前這台電腦裡的 AI Agent，更建議直接使用本頁的「安裝 OpenClaw」卡片，或在上方手動綁定 CLI 路徑。'),
  searchOpenclawTitle: _('本机已经装了 AI Agent？', 'Already installed AI Agent locally?', '本機已經安裝 AI Agent？'),
  searchOpenclawDesc: _('可以主动搜索本机常见安装位置，并从结果中选择当前要管理的 AI Agent CLI。', 'You can actively search common local install locations and choose which AI Agent CLI this panel should manage.', '可以主動搜尋本機常見安裝位置，並從結果中選擇目前要管理的 AI Agent CLI。'),
  searchOpenclawBtn: _('搜索 AI Agent', 'Search AI Agent', '搜尋 AI Agent'),
  ...
  dockerDefaultImagePlaceholder: _('例如 ghcr.io/openclaw/openclaw 或 1186258278/openclaw', 'e.g. ghcr.io/openclaw/openclaw or 1186258278/openclaw', '例如 ghcr.io/openclaw/openclaw 或 1186258278/openclaw'),
  ...
  searchOpenclawHint: _('搜索结果里点击「使用」即可绑定当前 CLI；若随后仍提示配置文件缺失，请到右侧配置卡里填写自定义 AI Agent 配置目录。', 'Click "Use" on a result to bind that CLI. If config is still missing afterwards, set the custom AI Agent config directory in the config card on the right.', '在搜尋結果中點擊「使用」即可綁定目前 CLI；若之後仍提示設定檔缺失，請到右側設定卡中填寫自定義 AI Agent 設定目錄。'),
  searchOpenclawScanning: _('正在搜索常见安装位置...', 'Searching common install locations...', '正在搜尋常見安裝位置...'),
  searchOpenclawEmpty: _('没有在常见路径中找到 AI Agent。你可以继续手动填写 CLI 路径，或直接使用本页的「安装 OpenClaw」卡片。', 'No AI Agent installation was found in common paths. You can keep entering the CLI path manually, or use the \"Install OpenClaw\" card on this page.', '沒有在常見路徑中找到 AI Agent。你可以繼續手動填寫 CLI 路徑，或直接使用本頁的「安裝 OpenClaw」卡片。'),
  searchOpenclawUsing: _('使用中...', 'Using...', '使用中...'),
  searchOpenclawSelectSuccess: _('已选择该 AI Agent CLI，正在重新检测...', 'Selected this AI Agent CLI, re-detecting...', '已選擇此 AI Agent CLI，正在重新檢測...'),
  searchOpenclawSelectFailed: _('选择失败: {err}', 'Selection failed: {err}', '選擇失敗: {err}'),
  searchOpenclawManualLabel: _('或者手动指定已有 AI Agent CLI / 安装目录', 'Or enter an existing AI Agent CLI / install directory manually', '或者手動指定既有 AI Agent CLI / 安裝目錄', 'または既存の AI Agent CLI / インストールディレクトリを手動指定'),
  ...
  searchOpenclawManualSaved: _('已绑定该 AI Agent CLI，正在重新检测...', 'Bound this AI Agent CLI, re-detecting...', '已綁定此 AI Agent CLI，正在重新檢測...', 'この AI Agent CLI を関連付けました。再検出しています...'),
  searchOpenclawManualNotFound: _('在这个位置没有找到可用的 AI Agent CLI，请确认路径正确。', 'No usable AI Agent CLI was found at this location. Please verify the path.', '在這個位置沒有找到可用的 AI Agent CLI，請確認路徑正確。', 'この場所に利用可能な AI Agent CLI が見つかりませんでした。パスを確認してください。'),
  wslWebHint: _('WSL 中使用 Web 版：', 'Use Web version in WSL:'),
  wslWebDesc: _('如果你的 AI Agent 本来就运行在 WSL 里，可打开 WSL 终端部署 AI Agent面板 Web 版：', 'If your AI Agent already runs inside WSL, open a WSL terminal and deploy AI Agent面板 Web there:', '如果你的 AI Agent 本來就運行在 WSL 裡，可開啟 WSL 終端部署 AI Agent面板 Web 版：'),
  ...
  dockerDesc: _('如果你的 AI Agent 本来就运行在容器里，可在容器内安装 AI Agent + AI Agent面板 Web 版：', 'If your AI Agent already runs inside a container, install AI Agent + AI Agent面板 Web there:', '如果你的 AI Agent 本來就運行在容器裡，可在容器內安裝 AI Agent + AI Agent面板 Web 版：'),
  remoteHint: _('远程服务器：', 'Remote server:', '遠程伺服器：'),
  remoteDesc: _('如果 AI Agent 装在远程服务器上，请 SSH 登录服务器后执行：', 'If AI Agent is installed on a remote server, SSH into that server and run:', '如果 AI Agent 裝在遠程伺服器上，請 SSH 登入伺服器後執行：'),
  ...
  promptCliMissing: _('AI Agent CLI 未安装', 'AI Agent CLI not installed', 'AI Agent CLI 未安裝'),
  promptCliOk: _('AI Agent CLI 已安装', 'AI Agent CLI installed', 'AI Agent CLI 已安裝'),
  promptConfigMissing: _('配置文件不存在', 'Config file not found', '設定檔案不存在'),
  promptConfigOk: _('配置文件正常: {path}', 'Config file OK: {path}', '設定檔案正常: {path}'),
  promptIntro: _('我在安装 AI Agent 时遇到问题，以下是当前检测状态：', 'I\'m having issues installing AI Agent. Here is the current detection status:', '我在安裝 AI Agent 時遇到問題，以下是目前檢測狀態：'),
  ...
  gitManualInstallHtml: _('<strong>Windows:</strong> 下载 <a href="https://github.com/agentai2026/AI-Agent" target="_blank" style="color:var(--accent)">git-scm.com</a> 安装包<br><strong>macOS:</strong> 在终端执行 <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">xcode-select --install</code> 或 <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">brew install git</code><br><strong>Linux:</strong> <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">sudo apt install git</code> 或 <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">sudo yum install git</code>', '<strong>Windows:</strong> Download from <a href="https://github.com/agentai2026/AI-Agent" target="_blank" style="color:var(--accent)">git-scm.com</a><br><strong>macOS:</strong> Run <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">xcode-select --install</code> or <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">brew install git</code><br><strong>Linux:</strong> <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">sudo apt install git</code> or <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">sudo yum install git</code>', '<strong>Windows:</strong> 下載 <a href="https://github.com/agentai2026/AI-Agent" target="_blank" style="color:var(--accent)">git-scm.com</a> 安裝包<br><strong>macOS:</strong> 在終端執行 <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">xcode-select --install</code> 或 <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">brew install git</code><br><strong>Linux:</strong> <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">sudo apt install git</code> 或 <code style="background:var(--bg-secondary);padding:2px 4px;border-radius:3px">sudo yum install git</code>'),
  ...
  installSuccess: _('AI Agent 安装成功', 'AI Agent installed successfully', 'AI Agent 安裝成功', 'AI Agent インストール成功', 'AI Agent 설치 성공'),
