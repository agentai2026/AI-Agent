...
export default {
  ...
  backendDownDesc: _('AI Agent面板 后端服务未运行，无法获取真实数据。', 'AI Agent面板 backend is not running, so live data is unavailable.', 'AI Agent面板 後端服務未執行，無法取得真實資料。', 'AI Agent面板 バックエンドが実行されていないため、実データを取得できません。', 'AI Agent面板 백엔드가 실행 중이 아니어서 실제 데이터를 가져올 수 없습니다.', 'Dịch vụ backend của AI Agent面板 chưa chạy nên không thể lấy dữ liệu thực.'),
  ...
  pageLoadFailedHint: _('如果问题持续出现，请尝试重新安装 AI Agent面板，或在 GitHub Issues 反馈。', 'If the problem persists, try reinstalling AI Agent面板 or report it on GitHub Issues.', '如果問題持續出現，請嘗試重新安裝 AI Agent面板，或到 GitHub Issues 回報。', '問題が続く場合は AI Agent面板 を再インストールするか、GitHub Issues で報告してください。', '문제가 계속되면 AI Agent面板을 다시 설치하거나 GitHub Issues에 제보하세요.', 'Nếu lỗi vẫn lặp lại, hãy thử cài lại AI Agent面板 hoặc báo trên GitHub Issues.'),
  ...
  upgradeOpenClaw: _('升级 AI Agent', 'Upgrade AI Agent', '升級 AI Agent', 'AI Agent をアップグレード', 'AI Agent 업그레이드', 'Nâng cấp AI Agent', 'Actualizar AI Agent', 'Atualizar AI Agent', 'Обновить AI Agent', 'Mettre à jour AI Agent', 'AI Agent aktualisieren'),
